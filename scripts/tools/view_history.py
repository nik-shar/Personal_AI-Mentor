"""
scripts/tools/view_history.py

View, search, and manage your AI Mentor conversation history & memory store.

Commands:
    uv run python scripts/tools/view_history.py                   # View recent conversation turns
    uv run python scripts/tools/view_history.py search <query>    # Semantic RAG search over past chats
    uv run python scripts/tools/view_history.py stats             # Show DB memory stats & open queued questions
    uv run python scripts/tools/view_history.py clear             # Clear conversation history (with confirmation)
"""
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage:")
        print("  uv run python scripts/tools/view_history.py           (list history)")
        print("  uv run python scripts/tools/view_history.py search X  (semantic search)")
        print("  uv run python scripts/tools/view_history.py stats     (memory stats)")
        print("  uv run python scripts/tools/view_history.py clear     (clear history)")
